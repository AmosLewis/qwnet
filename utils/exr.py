...
def write(filename, data, channel_names = None, precision = FLOAT, compression = PIZ_COMPRESSION, header = None):
...
class InputFile(object):
  ...
  def get(self, group = 'default', precision=FLOAT):
    ...
    matrix = np.zeros((len(channels),self.height, self.width), dtype=NP_PRECISION[str(precision)])
